...
>    再把 3 个索引 DROP 后重跑（覆盖「存量库缺索引」路径）也全部正确重建。
>    **新增 `server/sql/migrate-prod.sh`**：手册四步串成一条命令（备份→执行→校验→计划抽查，失败即停），
>    已在旧索引形状的临时库整体彩排通过。生产执行 = 服务器买好后 `bash server/sql/migrate-prod.sh <用户> <库>`。
>    **生产库当前不存在**（`docs/上线实施方案.md`：服务器/备案是 M0 阻断项），本机已尽到脚本级上限。
...
> 7. **追加（同日，owner 指示「先把小程序的所有搞定，ui 什么的」）**：全站 38 页逐页导航走查
>    （主包 7 页 + 分包 31 页；VIP/支付/手机号登录 5 页验证守卫弹回），全程 console **0 error/warn**。
>    抓到并修掉 4 个 UI 问题（见 CHANGELOG Fixed）：空态插画张冠李戴（gray/offline 共用断网图标，
>    全站 3 处）、凌晨时间口径两页打架、图块规格表补 4 角色使**主线裸值 4→0**、
>    两条搁置观感决策落地（金色压档 `#a58145`、做菜进度条一次性跳步提示）。
>    11 条前端门禁复跑全绿；`interaction-audit` A/B/C 全 0（R2 遗留的 13 处无反馈元素已在
>    前几轮清零，本轮复核确认）。⚠ 走查方法坑：navigateTo 叠到 11 层会静默失败（微信 10 层上限），
>    批量走查必须用 reLaunch。⚠ `automation_viewport_action --action remote`（真机调试）会把
>    自动化运行时挂起，退出需重开项目窗口。
> 5. **真机三项前置已打通**（模拟器仍不能代替真机）：`utils/env.js` 新增真机开发版分支——
>    非 devtools 平台走 `DEV_LAN_BASE`（预填 `http://192.168.1.83:9088`），真机预览从此能连本机后端；
>    运行时双分支实测（devtools→localhost / mock 真机→局域网）+ static-check 全绿。
>    `auto_preview` 已把开发版（主包 1.31MB）推到微信；操作步骤与三项核对清单见
>    `artifacts/launch-2026-09-25/status-launch-prep.md` §6。**三项本身仍要真机操作**。
...
### 第六轮 U1~U5（2026-09-20）：UI 细节第五批——照片裁切 / 图挂了 / 卡内边距 / 头像形状

> 前四批（字号吸附、骨架对齐、token 表面、小触点）已各自成 commit；这批是 owner 说"好多的 UI 细节没有把握好"之后
> 逐条量出来的收尾。判据全部是**量出来的**，不是看截图觉得像。

| 项 | 结果 | 证据：怎么量的 / 修复前测到什么 / 修复后测到什么 |
| --- | --- | --- |
| 大图框把菜裁掉一半 | ✅ | **怎么量**：`sips` 数源图比例——`assets/dishes` 19 张里 **16 张是 600×600 方图**（另 3 张 3:2），所以"框越扁、裁得越多"；再用 `automation_element_action --action size` 量实际渲染框。<br>**修复前**：社区配图 308×167px（=588×322rpx，**1.83:1**，方图纵向只剩 55%）、菜谱步骤图 266×124px（=515×240rpx，**2.15:1**，只剩 47%）、做菜模式大图 658×400rpx（1.65:1，只剩 61%）。一条鱼被拦腰裁掉就是这么来的。<br>**修复后**：三处分别 308×228px（1.35:1）、266×200px（1.33:1）、658×494rpx（1.33:1），方图纵向保留 74%~75%。规格进 `app.wxss` 的 `--photo-feed-h / --photo-step-h / --photo-focus-h / --photo-head-h`，改一处四页同步。<br>顺带把两处"骨架 ≠ 真卡"对齐：帖子详情骨架 360→462rpx（=真图高）、收藏页骨架 260→288rpx（=`.rc-ph` 真高），加载完不再跳版。 |
| 图挂出来是一个**尺寸完好的空洞** | ✅ | **怎么量**：把社区信息流第一条的 `photo` 用 `setData` 指到不存在的文件，截图。<br>**修复前**：那张卡留下 440rpx 高的纯白，和"这条帖子本来就没图"长得一模一样；全站 55 个 `<image>` 只有 2 个写了 `binderror`。<br>**修复后**：`app.wxss` 加一个 `.img-ph`——实测 WeChat 渲染层允许 `<image>` 出 `::before` 且画在内层图片之下，所以图正常时一个字都看不见（正反两面都截图验过），图 404 时元素透明、兜底"图片没加载出来"才露出来。<br>**为什么不用 `binderror` 铺开**：回调要知道"改哪个 data 路径"，等于给每个列表项串一条 key，30 个页面各写一遍、漏一个静默失效且看不出来；这个类只挂在类名上，没有"漏接"这种失败模式。已登记 5 处大图位（社区配图、菜谱步骤图、做菜模式、帖子详情头图、菜谱卡封面）。 |
| 同一屏两张卡的文字错开 | ✅ | **怎么量**：按"卡表面"这个角色扫（`border-radius: var(--r-card)` 的规则），列出全部 `padding`。<br>**修复前**：同一个卡表面全站写了 20 多种 padding，光左右就有 16/18/22/24/25/26/28/30/31/32/34/35/40/42/64；首页「许愿池」白卡左右 **28**、它下面那张深色主卡左右 **40** —— 两张卡左边缘齐平、里面的文字却错开 6px（截图上量得出来）。<br>**修复后**：只钉**左右**这一个轴（纵向留白跟内容节奏走，不该并）：`--pad-card: 28rpx` / `--pad-card-sm: 24rpx`，31 处改成引用规格表；首页两张卡的文字左边缘重新对齐（截图复核）。<br>确实要窄的两处（菜单三列统计卡、两列快捷卡）在同一行写「例外：原因」，不静默放过。 |
| 同一个人换个页面就换个头 | ✅ | **怎么量**：扫所有头像位的 `width` 与 `font-size`，算"字/框比"。<br>**修复前**：同一个"阿"字占框宽从 **36% 到 53%** 都有（36/84、32/88、36/92、32/72、26/72、19/36）；形状也不一致——社区列表是 26rpx 圆角方、点进他那条帖子变成**正圆**。<br>**修复后**：头像只有一个自由度（框宽），所以 `border-radius = 框宽 × --r-ava(0.31)`、`font-size = 框宽 × --ava-glyph(0.4) × --fs-mul`。0.31 不是拍的：它就是现有 26/84 与 62/200 两处实测值，也正是微信自己的头像比例。<br>截图复核：社区列表与帖子详情的"阿宁"现在是同一个形状、同一个字大小（±3% 内）。`--ava-hero: 208rpx` 同时收编了个人资料页那个裸值。 |
| 空态块高度"三种写法" | ⬠ **判定为误报，未改** | 这条是我上一批自己列的待办，量下来站不住：所谓 pantry 的 150rpx 其实是 `.af-sm { width: 150rpx }`（一列的宽度，不是空态高度）；home 的 `min-height: 72vh` 是**有意**把整页错误态在视口里居中，而它里面装的本来就是 `state-empty`（注释写着"与其它 13 个页面统一走 state-empty"）。`state-empty` 自己是内容驱动高度、全站共用一个组件。<br>顺手核实了两处看着像"自绘空态"的：`pkg-extra/import` 的 `.res-empty` 是"未识别到食材，可手动补充"的行内提示、home 的 `.mh-empty-*` 是"从第一道菜开始"的引导卡，都**不是**列表空态，角色不同、不该并。 |
| 门禁 25 → 29 项（每条都反向验过） | ✅ | 26 大图框比例（① token ÷ 实测容器宽 ≤ 1.45:1；② 五处大图位的 height 必须引用 `--photo-*-h`）；27 大图位必须挂 `img-ph`（含 `.img-ph::before` 得有 `content`，否则这个类只剩 `position:relative`）；28 卡表面左右内边距必须出自 `--pad-card*`，同行写「例外：原因」才放过；29 头像位的圆角/字号必须由框宽 token 推出。<br>反向验证：把 `--photo-feed-h` 改回 322rpx → 报"方图纵向只剩 55%"；把 `.pphoto` 高度写回裸值 → 报"没走规格表"；摘掉一个 `img-ph` → 报 1/1 处缺兜底；`.pavatar` 圆角改回 26rpx / 字号改回 36rpx → 各报一条。全部还原后 `✔`。<br>另：`interaction-audit` 的 A 类判据是"末位类名像图片就豁免"，`img-ph` 加在末尾正好把它顶掉（`rd-step-img` → `rd-step-img img-ph`），所以按角色把 `.img-ph` 加进豁免表，而不是靠类名写在第几位。 |
| 台账读数 | ✅ | 四条主线"没名字的裸值种数"：图块 4 / 圆角 32 / 加载态 3 / 空态 2（token 数 图块 10、圆角 10，比上批各 +1，都是**有角色名**才加的）；全站裸 rpx **3688 → 3660**；`pages/me` 的图块裸值 `88rpx` 消失、`pages/home`/`pages/shopping`/`pkg-extra/weekly-menu` 各掉一档裸圆角。 |
| 证据边界 | ⚠ | 全部结论来自 iOS 模拟器 + CSS 算术：`calc(var(--ava-row) * var(--r-ava))` 这类嵌套 var 的 calc 在**老 Android WebView（Chromium < 79）**上的表现没验过，模拟器这一档是绿的；真机若出现头像变直角，第一嫌疑是这行而不是 WXSS 写错。图片兜底 `::before` 同理——它依赖渲染层允许 `<image>` 有伪元素，实测成立但不是规范承诺。 |
