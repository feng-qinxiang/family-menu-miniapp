...
| 模块 | 能做什么 |
| --- | --- |
| 看板 | 用户/家庭/菜谱/帖子计数 + **待审帖子 / 待审评论 / 待处理举报 / 待处理反馈**（有积压标红，点击直达对应列表） |
| 举报审核 | 处理用户举报：下架 / 忽略；可按待处理/已下架/已忽略筛选 |
| 内容治理 | 帖子审核与下架：待审核 → 通过 / 下架；已下架可恢复 |
| 评论管理 | 评论审核：待审核 → 通过 / 驳回；软删除与恢复；可按帖子 ID 过滤 |
| 反馈工单 | 工单列表、受理、关闭、回复 |
| 导入审核 | 导入源待审队列：通过 / 驳回（带原因） |
| 菜谱治理 | 菜谱下架与恢复、按标题搜索 |
| 用户管理 | 检索用户、授予/撤销管理员、人工开通会员、封禁/解封 |
| 订单 | 订单查询（含金额、状态、支付方式）、关单、退款 |
| 审计日志 | 所有管理写操作留痕：谁、何时、对什么、做了什么、结果；支持关键字搜索 |
