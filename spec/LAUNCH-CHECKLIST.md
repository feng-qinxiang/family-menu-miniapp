...
| 大字模式漏接了 8 个页面（含做菜模式） | ✅ 修 1 + 其余钉成常驻提醒 | `app.wxss` 里 `.font-lg` 的注释自己写着「全站 36 页根节点挂 .font-lg」，实测**只有 29 页真挂了**。漏掉的包括**做菜模式**和菜谱详情——不白屏不报错，就是「设置里开了大字，这一页照样小字」，而做菜那屏手机在一臂外、手上有油，最需要大字。做菜模式已接上（根节点挂 `.font-lg` + `onLoad` 读 `font_scale`；实测存储设成 lg 后页面 `data.fontScale` 确实变 lg，验完已还原）。**其余 7 个故意没顺手改**：`recipe-detail` 的根是 `scroll-view`、`recipe-edit` 是 `.rc-seg` + `.rc-body` 并列，没有单一根可挂，要改得逐页决定包不包一层，属结构改动。已做成 `static-check` 第 15 项：只接一半（绑了类没读档 / 读了档没绑类）**判失败**，整页没接则每次运行列出清单，缺口不再靠人记。 |
