...
| 举报审核 | 处理用户举报：下架 / 忽略；可按待处理/已下架/已忽略筛选，导出 CSV |
| 内容治理 | 帖子审核与下架：待审核 → 通过 / 下架；已下架可恢复，导出 CSV |
| 评论管理 | 评论审核：待审核 → 通过 / 驳回；软删除与恢复；可按帖子 ID 过滤，服务端分页 + 全量导出 CSV |
| 反馈工单 | 工单列表、受理、关闭、回复，服务端分页 + 全量导出 CSV |
| 导入审核 | 导入源待审队列：通过 / 驳回（带原因） |
| 菜谱治理 | 菜谱下架与恢复、按标题搜索 |
| 用户管理 | 检索用户、授予/撤销管理员、人工开通会员、封禁/解封，服务端分页 + 全量导出 CSV |
| 订单管理 | 订单查询（金额、状态、支付方式）、关单、退款，服务端分页 + 全量导出 CSV |
| 审计日志 | 所有管理写操作留痕：谁、何时、对什么、做了什么、结果；服务端关键字搜索 + 分页 + 全量导出 CSV |
