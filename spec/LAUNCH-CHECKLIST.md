...
>
> **R1~R5 已完成**（止血四处状态缺陷 → 图标标准收口 → UI 台账 + 图块/头像规格表 → 做菜计时与跳步 →
> 做菜收尾端到端 + 弹层/深色逼近），证据在下面「第六轮 R1 / R2 / R3 / R4 / R5」五张表里。
> 待做：R6 后端读路径与索引、R7 写路径竞态与 404 语义、R8 购物清单 N+1 与时钟口径。
> R5 另外**新增一条留给 owner 的观感决策**：浅色档金色小字在 `--paper-2` 上只有 2.71:1（73 处），
> 见 §8「浅色档的金色小字要不要压暗」。
>
