...
Page({
  ...
  async loadReports() {
    ...
    try {
      const [pendingRaw, reviewedRaw, ignoredRaw] = await Promise.all([
        getCommunityReports('PENDING'),
        getCommunityReports('REVIEWED'),
        getCommunityReports('IGNORED')
      ]);
      const handled = []
        .concat(Array.isArray(reviewedRaw) ? reviewedRaw : [])
        .concat(Array.isArray(ignoredRaw) ? ignoredRaw : []);
      this.setData({
        pending: this.normalize(pendingRaw, 'PENDING'),
        reviewed: this.normalize(handled, 'REVIEWED'),
        loading: false
      });
    } catch (e) {
