...
// 前端这一处是标签池的唯一来源（AVOID_TAGS 直接由 key 派生：家庭页的标签池、菜谱页的筛选、
// 资料页的偏好三处都从这里取）。
// ⚠ 「今日推荐」在服务端过滤（列表要本地即时筛选，推荐要在后端算），所以
// server/.../service/AvoidTagFilter.java 里有一份同内容的拷贝；
// 两份必须逐键一致，这一点由 AvoidTagFilterTests#keywordDictionaryStaysInSyncWithMiniappConstants
// 直接读本文件比对守住——改这里不同步改那边，CI 会红。
const AVOID_KEYWORDS = {
  // 「川菜」整个菜系不算辣味关键词：红烧肉也标着川菜，却一点不辣——
  // 忌口要挡的是"辣"这道味道，不是把一个菜系全家桶从推荐里抹掉。
  '辣': ['辣', '麻婆', '水煮', '香辣', '麻辣'],
  '香菜': ['香菜', '芫荽'],
