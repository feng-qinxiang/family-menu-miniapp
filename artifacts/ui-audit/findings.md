...
## 大字模式收口 + 静默失败清缴（2026-09-19）
- 重新实测全站（不再采信本文旧结论）：`.wxss` 共 **776 处 `font-size`**，其中 773 处由
  `var(--fs-*)` / `calc(Xrpx * var(--fs-mul, 1))` 驱动，另 3 处天然跟随父级
  （`var(--tb-fs)`、`0.62em`、`font-size: inherit`）。**真正裸写 rpx 的只剩 2 处**，
  都在 community 发帖配图区：`.pf-img-del`（28rpx ×）与 `.pf-img-add`（44rpx ＋）。
  → 阶段 3 / 阶段 6 写的「~400 处静态 rpx 不缩放」在本文件「残留项清缴 第二轮」就已作废，
    是同一文档内部自相矛盾，不是未完成。
- 上述 2 处已补齐，但**不是简单乘系数**：× 是 44rpx 圆内的字形，只放大字会撑破圆，
  故连圆形与 88rpx 命中区一起等比放大；＋ 的 132rpx 方格必须与 `.pf-img` 同宽才能对上图片网格，
  所以只放大字形。改后全站 `font-size` 零裸写。
- 已把该纪律**钉成 CI 阻断项**：`static-check.js` 第 8 项（扫裸 rpx/px 字号，跳过注释与跟随父级写法）。
  探针验证过它「能红」：植入 1 个真漏 + 4 类诱饵（跨行注释、行内注释、拆分声明、em/token 写法），
  结果精确报 2 处、零误报。此前只靠 app.wxss 注释提醒，下一张新页面就会破功。
- 顺手清掉一类更隐蔽的产品级 bug——**写操作被 `.catch(() => {})` 吞掉**：
  `cook-mode` 取消评分分支的做菜记录（失败仍跳记录页，用户以为记上了）、
  `recipe-detail` 的自动销愿（提示「愿望达成」但心愿仍在列表）。前者改为提示，后者按项目既有
  「尽力而为但要留痕」约定改为 console.warn。其余 7 处静默 catch 经逐条判断是读路径或刻意不阻断，维持。
- 更正 LAUNCH-CHECKLIST §8 一条会害人的指引：运营者占位符被写成在
  `pages/legal/privacy/index.js`（该路径不存在），实际唯一来源是 `utils/legal-config.js:13,15`，
  且 privacy/terms/help-faq 三页都只引用它——按旧指引改会漏改并带着【】提交被驳回。
- ⚠️ 未做：本会话微信开发者工具 MCP 未连接，community 发帖配图区两处几何改动**未经真实渲染验证**，
  需在有大字档/标准档两种状态下实拍一次（本文工程备注：批量改 wxss 后需重启项目再验）。

## 交互体检从"只报告"升为"A 类阻断"（2026-09-19 同日续）
- 该脚本此前在 CI 里是 `continue-on-error: true` 且自身无退出码 —— 等于一条永远不会失败的门禁，
  只在报告里躺着。现按可判定性分级：
  A 类（绑事件却没按下反馈）纯机械、无白名单、当前 0 处 → exit 1 且 CI 真阻断；
  B / C 靠类名与尺寸启发式（各有 4 / 2 处已确认误报白名单）→ 继续只报告，不假装精确。
- 反向验证：临时页面无反馈元素 1 个 + 两个诱饵（`tap-scale`、`hover-class`），脚本精确报 1 处、
  零误报，进程以 code=1 退出；删除探针后 A=0 且不再调用 exit。四类门禁全绿（285 文件 / 48 模板）。
- 顺带更正本文件与脚本头注释里 `B_OK` / `C_OK` 的过时名字（实际是 `B_CONFIRMED` / `C_CONFIRMED`）。

## 深色模式裸黑描边（2026-09-19 同日续）
- 按属性筛全站 WXSS 的裸色字面量（只看会随主题翻转的 border / color，排除 box-shadow 与照片上的黑底白字胶囊）：
  23 处 rgba(0,0,0,…) 里 4 处是「压在图片上的黑 scrim + 白字」两档都成立，属正确用法；
  **真缺陷 3 处**：`kitchen .k-tbtn.ghost`、`kitchen .k-act.ghost`（rgba(0,0,0,.14) 描边）、
  `community .pf-img-add`（rgba(0,0,0,.22) 虚线）——深底上叠更黑 = 幽灵按钮/加图入口在深色档直接消失，
  三处全在核心链路（厨房总控、社区发帖）。
- 修法：一律换 `var(--line-deep)`。选它而非 `--c-border` 是因为浅色档 #d5c9b8 与原
  半透明黑叠加后的实测像素（≈#d7d4d0 / #c4c1bd）几乎一致，改完浅色不动、深色拿到 #3a3a3a。
- 机制已查证：深色不是靠 theme.json 注入 WXSS，而是 `app.wxss` 的 `@media (prefers-color-scheme: dark)`
  块里重定义 `--themeLine/--themeLineDeep/--themeBorder` 等（第 182-185 行），
  `--line-deep: var(--themeLineDeep, #d5c9b8)` 因此会真的翻转 —— 曾疑似"描边 token 深色不翻转"，
  核对后是本方案正常工作，未误改。
- 新增 `static-check.js` 第 9 项把它钉住：`border|color` 用裸黑即失败。
  反向验证：探针含 2 真漏 + 5 类诱饵（跨行注释、黑 scrim+白字、白描边、token 描边、box-shadow、
  `border-radius` 里含 "border" 字样），精确报 2 处、零误报，进程 code=1；删探针后 285 文件全绿。
  为共带去注释行扫描器，把第 8 项重构为共用 `wxssCodeLines()`，行为不变（重跑同样只报该报的）。
- ⚠️ 证据边界：本次 3 处描边修复依据是"token 在两档下的实际色值 + 色彩叠加计算"，
  深色下的**观感仍未经真机/模拟器渲染确认**（本会话 wechatide MCP 未连接）。

### 同类的反向缺陷：恒定底上用了翻转色（第 4 处）
- `recipe-detail .rd-video-play` 是压在视频封面上的**恒定白**圆（两档都不翻转），
  但三角 `.rd-ic-play-lg` 用了 `var(--ink-deep)` —— 深色档它翻成 #e2e2e2，
  等于白圆里画一个白三角，播放键看着是空的。改为恒定的 `var(--anchor)`(#3a2e23，注释即"两主题不翻转")。
  这是本轮唯一"底色恒定、前景却跟着主题翻"的一例，和前 3 处（底色翻、前景写死）方向相反。
- 全站扫过同类：`cook-mode` 幽灵钮(白 alpha 描边)在 `--cook-bg` 上——该底色浅 #1a1410/深 #111111，
  两档都是深色，故白 alpha 正确；`weekly-menu` 七日条、`cook-log` 热力格同理（其 hero 用恒定白字，
  浅色档走查早已通过即为证）；`ad-banner .notice-kicker` 用 `--pop-deep`（浅 #b3381f/深 #d94b30）
  压在 68% 白胶囊上，两档均可读。→ 除已修的 4 处外无同类缺陷。
- 反白背景方向另扫了一遍：全站 `background` 写死纯白/近白 **0 处**（P0 色值 token 化已覆盖），
  147 处裸白全是"彩色底/照片底上的白字或白描边"，属正确用法。
