...
已命名的规格：图块 9 个 token、圆角 9 个 token。这两个数**不该无脑涨**——每加一个 token 都要能说出它是哪个角色。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `pages/home` | — | --r-card / --r-chip / --r-md / --r-pill / --r-sheet / 0 0 36rpx 36rpx / 12rpx / 20rpx / 28rpx / 2rpx / 3rpx / 40rpx / 44rpx / 4rpx / 50% | 骨架屏 | state-empty | 247 | 0 | 0 | 4 | — | — |
| `pages/menu` | --dish-thumb | --r-card / --r-chip / --r-lg / --r-md / --r-pill / --r-sheet / --r-xl / --r-xs / 0 0 14rpx 14rpx / 12rpx / 14rpx 14rpx 0 0 / 25rpx / 34rpx / 50% / 6rpx | 骨架屏 | state-empty | 190 | 0 | 0 | 6 | — | — |
| `pages/recipes` | — | --r-card / --r-chip / --r-md / --r-pill / --r-sheet / --r-sm / --r-xs / 26rpx / 34rpx / 38rpx / 50% | 骨架屏 | state-empty | 111 | 1 | 0 | 0 | — | — |
| `pages/shopping` | --tile-chip | --r-md / --r-pill / --r-sheet / --r-sm / --r-xs / 12rpx / 18rpx / 22rpx / 34rpx / 50% | 骨架屏 | state-empty | 123 | 0 | 0 | 0 | — | — |
| `pkg-extra/favorites` | --tile-grid | --r-card / --r-md / 12rpx / 50% / 6rpx | 骨架屏 | state-empty | 37 | 0 | 0 | 0 | — | — |
| `pkg-extra/recipes/search` | — | --r-card / --r-chip / --r-xs / 2rpx / 4rpx / 50% | state-loading | state-empty | 60 | 0 | 0 | 4 | — | — |
| `pkg-extra/weekly-menu` | --tile-row-sm | --r-card / --r-pill / --r-sheet / --r-xs / 25rpx / 28rpx / 34rpx / 4rpx / 50% | state-loading | state-empty | 79 | 0 | 0 | 0 | — | — |
...
| 页面 | 图块尺寸 | 圆角取值 | 加载态 | 空态 | 裸 rpx | 裸 px | emoji | CSS 图标 | 未声明组件 | 置了没人读的状态位 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `pages/community` | --ava-row / --tile-row-sm / 132rpx | --r-card / --r-chip / --r-pill / --r-sheet / --r-xs / 0.2em 0.2em 0.2em 0.06em / 12rpx / 20rpx / 22rpx / 26rpx / 28rpx / 4rpx / 50% | 骨架屏 | state-empty | 163 | 0 | 0 | 10 | — | — |
| `pkg-extra/community/post-detail` | --ava-cmt / --ava-head / 138rpx | --r-card / --r-chip / --r-pill / 12rpx / 14rpx / 26rpx / 2rpx / 50% | 骨架屏 | state-empty | 93 | 0 | 0 | 7 | — | — |

### 冰箱

| 页面 | 图块尺寸 | 圆角取值 | 加载态 | 空态 | 裸 rpx | 裸 px | emoji | CSS 图标 | 未声明组件 | 置了没人读的状态位 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `pages/pantry` | — | --r-card / --r-md / --r-sheet / --r-sm / --r-xs / 19rpx / 23rpx / 27rpx / 2rpx / 34rpx / 38rpx / 3rpx / 50% / 50% 50% 45% 45% / 6rpx / 6rpx 6rpx 10rpx 10rpx | state-loading | state-empty | 105 | 0 | 0 | 0 | — | — |

### 未归类

| 页面 | 图块尺寸 | 圆角取值 | 加载态 | 空态 | 裸 rpx | 裸 px | emoji | CSS 图标 | 未声明组件 | 置了没人读的状态位 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `pages/me` | --tile-row-sm / 88rpx | --r-card / --r-chip / --r-lg / --r-md / --r-pill / --r-sheet / --r-xs / 11rpx / 12rpx / 14rpx / 26rpx / 2rpx / 50% / 6rpx 6rpx 10rpx 10rpx | 骨架屏 | 自绘 | 131 | 0 | 0 | 0 | — | — |
...
| `pkg-extra/family/create` | 200rpx / 62rpx / 73rpx | --r-sheet / --r-sm / --r-xs / 0 0 4rpx 4rpx / 2rpx / 4rpx 0 0 0 / 4rpx 4rpx 0 0 / 50% / 50% 50% 50% 0 / 62rpx | 无 | 无 | 107 | 0 | 0 | 6 | — | — |
| `pkg-extra/family/invite` | — | --r-pill / --r-xs / 23rpx / 2rpx / 31rpx / 35rpx / 4rpx / 50% / 54rpx / 6rpx | 无 | 无 | 76 | 0 | 0 | 3 | — | — |
| `pkg-extra/family/join` | 116rpx / 46rpx / 72rpx | --r-md / --r-xs / 2rpx / 50% | 无 | state-empty | 92 | 0 | 0 | 0 | — | — |
...
| `pkg-extra/me/profile-edit` | 208rpx | --r-pill / --r-xs / 22rpx / 50% / 6rpx | 无 | 无 | 68 | 0 | 0 | 1 | — | — |
| `pkg-extra/me/settings` | — | --r-card / --r-md / --r-pill / 2rpx / 50% | 无 | 无 | 35 | 0 | 0 | 0 | — | — |
| `pkg-extra/payment/checkout` | — | --r-chip / --r-sm / 50% / 6rpx 6rpx 0 0 | 无 | 无 | 62 | 0 | 0 | 0 | — | — |
| `pkg-extra/payment/success` | — | --r-pill / 50% | 无 | 无 | 37 | 0 | 0 | 0 | — | — |
| `pkg-extra/vip` | — | --r-card / --r-pill / --r-sheet / --r-xs / 18rpx / 22rpx / 50% | 无 | 无 | 77 | 1 | 0 | 0 | — | — |
...
| `components/recipe-card` | --dish-thumb | --r-card / --r-md / --r-pill / --r-xs / 50% | 无 | 无 | 63 | 0 | 0 | 0 | — | — |
